...
# Function for classification methods with a tuning grid
class_nnet<- function(method, grid, ctrl){
  model<- train(x = recruit_train_n[,1:16], 
               y = recruit_train_n$HiringDecision,
               method = method,
               preProc = c("center", "scale"),
...
glmnGrid <- expand.grid(alpha = seq(0, 0.2, length = 10),
...
```{r}
set.seed(720)
glmn_train<- class_grid("glmnet", glmnGrid, ctrl)
...
```{r}
set.seed(720)
nnet_train<- class_nnet("nnet", nnet_grid, ctrl)
...
```{r}
set.seed(720)
cart_train<- class_grid("rpart", cart_grid, ctrl)
...
plot(glmn_roc, type = "s", add = TRUE, col = 'brown', legacy.axes = TRUE)
...
legend("bottomright", legend=c(paste("LR (AUC =", round(glm_roc$auc,4), ")"),
                               paste("Penalized LR (AUC =", round(glmn_roc$auc,4), ")"),
       ...
       col=c("red", "brown", "blue", "pink", "yellow", "green", "purple", "orange"), lwd=2)
```
...
### Variable Importance
```{r}
glm_imp<- varImp(glm_train)
plot(glm_imp, top = 10, main = "CV GLM Variable Importance")

glmn_imp<- varImp(glmn_train)
plot(glmn_imp, top = 10, main = "CV Penalized LR Variable Importance")

nb_imp<- varImp(nb_train)
plot(nb_imp, top = 10, main = "CV NB Variable Importance")

knn_imp<- varImp(knn_train)
plot(knn_imp, top = 10, main = "CV KNN Variable Importance")

nnet_imp<- varImp(nnet_train)
plot(nnet_imp, top = 10, main = "CV NNET Variable Importance")

cart_imp<- varImp(cart_train)
plot(cart_imp, top = 10, main = "CV CART Variable Importance")

c50_imp<- varImp(c50_train)
plot(c50_imp, top = 10, main = "CV C5.0 Variable Importance")

rf_imp<- varImp(rf_train)
plot(rf_imp, top = 10, main = "CV RF Variable Importance")
```
...
testResults <- data.frame(
  ...
  glmn = predict(glmn_train, recruit_test_n[, 1:16]),
...
confusionMatrix(testResults$obs, testResults$glmn)
